...
from cirtorch.datasets.datahelpers import default_loader, imresize,im_resize
...
class ImagesFromList(data.Dataset):
    ...
    def __init__(self, root, images, imsize=None, bbxs=None, transform=None, loader=default_loader):
        ...
        print("First: ", images_fn[0])
        print("Last: ", images_fn[-1])

        if len(images_fn) == 0:
            print("Error: ", images_fn)
    ...
    def __getitem__(self, index):
        ...
        if self.imsize is not None:
            if self.bbxs is not None:
                img = im_resize(img, self.imsize * max(img.size) / imfullsize)
            else:
                img = im_resize(img, self.imsize)
