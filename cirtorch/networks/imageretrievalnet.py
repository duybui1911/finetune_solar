...
def extract_db_array(net, images, transform, pad=0.2,ms=[1], msp=1 , print_freq=10, bs = 1):
    ...
    with torch.no_grad():
        ...
        for i, input in enumerate(loader):
            ...
            len_input = len(input)
            if pad > 0:
                avg_features = extract_mp(net, input, pad)
                vecs[:, i*len_input:(i+1)*len_input] = avg_features

            else:
                if len(ms) == 1 and ms[0] == 1:
                    forwarded = net(input)
                    vecs[:, i*len_input:(i+1)*len_input] = forwarded

                else:
                    vecs[:, i*len_input:(i+1)*len_input], _ = extract_ms(net, input, ms, msp)
...
def extract_vectors(net, images, image_size, transform, bbxs=None,pad=0,ms=[1], msp=1, print_freq=10, bs=1):
    # moving network to gpu and eval mode
    net.cuda()
    net.eval()
    ...
    with torch.no_grad():
        ...
        for i, input in enumerate(loader):
            ...
            if pad > 0:
                avg_features = extract_mp(net, input, pad)
                vecs[:, i*bs:(i+1)*bs] = avg_features

            else:
                if len(ms) == 1 and ms[0] == 1:
                    forwarded = net(input)
                    vecs[:, i*bs:(i+1)*bs] = forwarded

                else:
                    vecs[:, i*bs:(i+1)*bs], _ = extract_ms(net, input, ms, msp)
...
def extract_vectors_by_arrays2(net, images, transform,pad=0,ms=[1], msp=1, batch_size = 1):
    loader = torch.utils.data.DataLoader(
        ImagesFromDataList(images=images,transform=transform),
    ...
    with torch.no_grad():
        vecs = torch.zeros(net.meta['outputdim'], len(images))
        vecs_atsacle1 = torch.zeros(net.meta['outputdim'], len(images))
        for i, input in enumerate(loader):
            # input = input.cuda()
            # len_input = len(input)
            if pad > 0:
                # forwarded = net(input)
                # forwarded = forwarded.data
                # vecs[:, i*batch_size:(i+1)*batch_size] = forwarded

                avg_features = extract_mp(net, input, pad)
                vecs_atsacle1[:, i*batch_size:(i+1)*batch_size] = avg_features
                vecs[:,i*batch_size:(i+1)*batch_size] = avg_features
                ...
                if len(ms) == 1 and ms[0] == 1:
                    ...
                    vecs[:, i*batch_size:(i+1)*batch_size] = forwarded
                    vecs_atsacle1[:, i*batch_size:(i+1)*batch_size] = forwarded
                else:
                    vecs[:, i*batch_size:(i+1)*batch_size], vecs_atsacle1[:, i*batch_size:(i+1)*batch_size] = extract_ms(net, input, ms, msp)
    return vecs, vecs_atsacle1


def extract_vectors_by_arrays(net, images, transform,pad=0.2,ms=[1], msp=1, batch_size = 1):
    loader = torch.utils.data.DataLoader(
        ImagesFromDataList(images=images,transform=transform),
        batch_size=batch_size, shuffle=False, num_workers=1
    )
        ...
        for i, input in enumerate(loader):
            ...
            if pad > 0:
                if len(ms) == 1 and ms[0] == 1:
                    avg_features = extract_mp(net, input, pad)
                    vecs[:, i*len_input: (i+1)*len_input] = avg_features
                    vecs_atsacle1[:, i*len_input:(i+1)*len_input] = avg_features

                else:
                    vecs_atsacle1[:,i*batch_size: (i+1)*batch_size] = extract_mp(net, input, pad)
                    avg_features_ms = torch.zeros(net.meta['outputdim'],len_input)
                    ...
                    for s in ms:
                        ...
                        feature_t_avg = extract_mp(net, input_t,pad)
                        if s == 1 and ms_len < 3 :
                            avg_features_ms += feature_t_avg
                        if s!= 1:
                            avg_features_ms += feature_t_avg


                    if ms_len >= 3 :
                        avg_features_ms /= (len(ms) -1)
                    else:
                        avg_features_ms /= len(ms)
                    avg_features_ms /= avg_features_ms.norm()

                    vecs[:, i*batch_size: (i+1)*batch_size] = avg_features_ms
              ...
              if len(ms) == 1 and ms[0] == 1:
                  ...
                  vecs[:, i*len_input:(i+1)*len_input] = forwarded
                  vecs_atsacle1[:, i*len_input:(i+1)*len_input] = forwarded
              else:
                  vecs[:, i*len_input:(i+1)*len_input], vecs_atsacle1[:, i*len_input:(i+1)*len_input] = extract_ms(net, input, ms, msp)
    return vecs, vecs_atsacle1
...
def extract_ms(net, input, ms, msp):
    n_images = len(input)
    v = torch.zeros(net.meta['outputdim'], n_images)
    v_atscale1 = torch.zeros(net.meta['outputdim'], n_images)
    ms_len = len(ms)
    for s in ms:
        if s == 1:
            input_t = input.clone()
            ...
            input_t = nn.functional.interpolate(input, scale_factor=s, mode='bilinear', align_corners=False)
        if s == 1:
            v_atscale1 = net(input_t).pow(msp).cpu().data
            if ms_len < 3:
                v += v_atscale1
        else:
            v += net(input_t).pow(msp).cpu().data
    if ms_len < 3:
        v /= len(ms)
    else:
        v /= (len(ms) -1 )
...
def extract_mp(net, input_tensors, pad):
    b, c, h, w = input_tensors.shape
    pad_h = int(pad * h)
    pad_w = int(pad * w)
    avg_features = torch.zeros(b, 2048)

    img_multy_list = torch.zeros((b,5,c,h,w))
    for i,input_tensor in enumerate(input_tensors):
        # print(input_tensor.shape)
        img_multy = torch.zeros((5,c,h,w))
        img_multy[0,:,:,:] = input_tensor

        img_pad_top = input_tensor.detach().clone()
        img_pad_top[:,:pad_h,:] = 0
        img_multy[1,:,:,:] = img_pad_top

        img_pad_bot = input_tensor.detach().clone()
        img_pad_bot[:,h-pad_h:, :] = 0
        img_multy[2,:,:,:] = img_pad_bot

        img_pad_left = input_tensor.detach().clone()
        img_pad_left[:,:,:pad_w] = 0
        img_multy[3,:,:,:] = img_pad_left

        img_pad_right = input_tensor.detach().clone()
        img_pad_right[:,:,w - pad_w:] = 0
        img_multy[4,:,:,:] = img_pad_right
        img_multy_list[i, :,:,:,:] = img_multy


    for i, img_multy in enumerate(img_multy_list):

        img_multy = img_multy.cuda()
        output = net(img_multy)
        output = output.T

        avg_features_i  = sum(output) / len(output)
        avg_features_i = avg_features_i.data
        avg_features[i,:] = avg_features_i
    avg_features = avg_features.T
