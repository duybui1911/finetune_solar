...
class ImagesFromList(data.Dataset):
    ...
    def __init__(self, root, images, imsize=None, bbxs=None, transform=None, loader=default_loader, mode='train',  pred_geom=False,net=None):
    ...
    def __getitem__(self, index):
        ...
        #     y_new= self.bbxs[index][1]-int((w_-temp_)/2)
        #     if(y_new<0):
        #         y_new=0
        # else:
        #     temp_=w_
        #     w_=h_
        #     x_new= self.bbxs[index][0]-int((h_-temp_)/2)
        #     if(x_new<0):
        #         x_new=0
        ...
        if self.mode == 'train':
            ...
            if self.transform is not None:
                ...
                # bbxs_=list(self.bbxs[index])
                # print("bbxs_",bbxs_)
                # bbxs_[0]= (bbxs_[0]/size_original[1]) * size_input[1]
                # bbxs_[1]= (bbxs_[1]/size_original[0]) * size_input[0]
                # bbxs_[2]= (bbxs_[2]/size_original[1]) * size_input[1]
                # bbxs_[3]= (bbxs_[3]/size_original[0]) * size_input[0]
                # # self.net.bbxs = bbxs_
